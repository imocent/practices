...
//                console.log("winHeight = " + winHeight);
    ...
    var dtree = {
        ...
        getCheckbarNodesParam: function (dTree) {  // 获取复选框选中值
            ...
            return dTree.getCheckbarNodesParam();
        },
        getCheckedIds: function (dTree) { // 获取复选框选中值的ID
            var NodesParam = this.getCheckbarNodesParam(dTree), checkIds = [];
            if (NodesParam && NodesParam.length > 0) {
                for (var i = 0; i < NodesParam.length; i++) {
                    var param = NodesParam[i];
                    checkIds.push(param["nodeId"]);
                }
            }
            return checkIds;
        },
