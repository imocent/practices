...
public class JsonRepair {
    ...
    private static final HashMap<Character, String> CONTROL_CHARACTERS = new HashMap<>();
    private static final HashMap<Character, Character> ESCAPE_CHARACTERS = new HashMap<>();
     ...
     */
    public static final String getInstance(String text) {
        return new JsonRepair(text).parse();
    }
    ...
    private String parse() throws RuntimeException {
        parseMarkdownCodeBlock(new String[]{"```", "[```", "{```"});

        boolean processed = parseValue();
        if (!processed) {
            JsonRepairException("Unexpected end of json string", text.length());
        }
        ...
        JsonRepairException("Unexpected character '" + getChar(i) + "'", i);
    ...
    private boolean parseValue() {
        parseWhitespaceAndSkipComments();
        boolean processed = parseObject() || parseArray() || parseString() || parseNumber() || parseKeywords() || parseUnquotedString(false) || parseRegex();
        parseWhitespaceAndSkipComments();
    ...
    private boolean parseObject() {
        if (getChar(i) == '{') {
            ...
            while (i < text.length() && getChar(i) != '}') {
                ...
                if (!processedKey) {
                    if (getChar(i) == '}' || getChar(i) == '{' || getChar(i) == ']' || getChar(i) == '[' || i >= text.length()) {
                        // repair trailing comma
                        StringUtils.stripLastOccurrence(output, ",");
                    } else {
                        JsonRepairException("Object key expected", i);
                    }
                    break;
                }
                ...
                if (!processedColon) {
                    if (StringUtils.isStartOfValue(getChar(i)) || truncatedText) {
                        ...
                        JsonRepairException("Colon expected", i);
                    }
                }
        ...
        if (StringUtils.isQuote(getChar(i))) {
            ...
            while (true) {
                ...
                if (isEndQuote.test(getChar(i))) {
                    ...
                    if (stopAtDelimiter || i >= text.length() || StringUtils.isDelimiter(getChar(i)) || StringUtils.isQuote(getChar(i)) || StringUtils.isDigit(getChar(i))) {
                    ...
                    if (escapeChar != null) {
                        ...
                        if (j == 6) {
                            ...
                            JsonRepairException(text.substring(i, Math.min(i + 6, text.length())), i);
                        }
                    } else {
                    ...
                    if (ch == '"' && getChar(i - 1) != '\\') {
                        ...
                        if (!StringUtils.isValidStringCharacter(ch)) {
                            JsonRepairException("Invalid character '" + ch + "'", i);
                        }
                        str.append(ch);
                        i++;
                    }
    ...
    private boolean parseUnquotedString(boolean isKey) {
        ...
        while (i < text.length() && !StringUtils.isUnquotedStringDelimiter(getChar(i)) && !StringUtils.isQuote(getChar(i)) && (!isKey || getChar(i) != ':')) {
            i++;
        }
    ...
    private void repairNumberEndingWithNumericSymbol(int start) {
        // repair numbers cut off at the end this will only be called when we end after a '.', '-', or 'e' and does not
    ...
    private void JsonRepairException(String message, int index) throws RuntimeException {
        throw new RuntimeException(String.format("Invalid unicode character '%s' at position %s", message, index));
    }
}